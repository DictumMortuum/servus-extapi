...
func (c *Client) getPasswordKey() (*rsa.PublicKey, error) {
	...
	key, err := util.GenerateRsaKey(passKey.Result.Password)
	if err != nil {
		return nil, err
	}
...
func (c *Client) getSessionKey() (*rsa.PublicKey, uint, error) {
	...
	key, err := util.GenerateRsaKey(passKey.Result.Key)
	if err != nil {
		return nil, 0, err
	}
...
func (c *Client) doEncryptedPost(path string, params EndpointArgs, body []byte, isLogin bool, result interface{}) error {
	encryptedData, err := util.AES256Encrypt(string(body), *c.aes)
	if err != nil {
		return err
	}
	...
	if len(sign) > 53 {
		first, _ := util.EncryptRsa(sign[:53], c.rsa)
		second, _ := util.EncryptRsa(sign[53:], c.rsa)
		sign = fmt.Sprintf("%s%s", first, second)
	} else {
		sign, _ = util.EncryptRsa(sign, c.rsa)
	}
	...
	decoded, err := util.AES256Decrypt(req.Data, *c.aes)
	if err != nil {
		return err
	}
